...
import { isPageStyle } from '../types/ui';
import type { PageStyle } from '../types/ui';

const ALTERNATE_STYLES = new Set<PageStyle>([
  'graffiti',
  'oldschool',
  'pixels',
  'luxe',
...
function rememberStyle(style: PageStyle): void {
  try {
...
function ensureStyleSheet(style: PageStyle): Promise<HTMLLinkElement | null> {
...
// Mirrors the hydration-safe pattern in useTheme.ts.
function currentStyle(): PageStyle {
  if (typeof document === 'undefined') return 'default';
  const style = document.documentElement.dataset.style;
  return style && isPageStyle(style) ? style : 'default';
}

export default function useStyle() {
  // Start from the value the server renders ('default') so hydration matches,
  // then adopt the real style right after mount.
  const [style, setStyleState] = useState<PageStyle>('default');
  ...
  const setStyle = useCallback((nextStyle: PageStyle) => setStyleState(nextStyle), []);
