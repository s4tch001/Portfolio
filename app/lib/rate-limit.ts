...
function store(): ReturnType<typeof getStore> | null {
  try {
...
function fresh(timestamps: number[] | null | undefined): number[] {
  const cutoff = Date.now() - WINDOW_MS;
  return (timestamps || []).filter((t) => t > cutoff);
}

async function read(ip: string): Promise<number[]> {
  const s = store();
  if (!s) return fresh(memory.get(ip));
  try {
    const timestamps = await s.get(keyFor(ip), { type: 'json' });
    return fresh(Array.isArray(timestamps) ? timestamps.filter(Number.isFinite) : []);
  } catch {
...
async function write(ip: string, timestamps: number[]): Promise<void> {
  const s = store();
  if (!s) {
    if (memory.size >= MAX_TRACKED_IPS && !memory.has(ip)) {
      const oldestIp = memory.keys().next().value;
      if (oldestIp !== undefined) memory.delete(oldestIp);
    }
